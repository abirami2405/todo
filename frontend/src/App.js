...
// Mock API services (replace with your actual API endpoints)
const authAPI = {
  getCurrentUser: async () => {
    const token = localStorage.getItem('token');
    if (!token) throw new Error('No token');
    // Mock user data
    return { name: 'Demo User', email: 'demo@example.com', id: '1' };
  },
  googleLogin: () => {
    // Mock OAuth - in real app, redirect to OAuth provider
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#3e2723] via-[#5d4037] to-[#795548] flex items-center justify-center p-4">
      <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 w-full max-w-md shadow-2xl border border-white/20">
        <div className="text-center mb-8">
          <div className="w-24 h-24 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-4">
            <CheckCircle className="w-12 h-12 text-[#ffccbc]" />
          </div>
          <h1 className="text-4xl font-bold text-[#ffccbc] drop-shadow-sm mb-2">Tickify</h1>
          <p className="text-[#d7ccc8] text-lg">Tick it off, stress off</p>
        </div>
...
const Dashboard = ({ user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#3e2723] via-[#5d4037] to-[#795548]">
      {/* Header */}
      <header className="bg-white/10 backdrop-blur-lg shadow-lg border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-[#ffccbc] rounded-lg flex items-center justify-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                onClick={handleLogout}
                className="flex items-center space-x-2 px-4 py-2 text-[#d7ccc8] hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all"
              >
                <LogOut className="w-4 h-4" />
                <span className="hidden sm:inline">Logout</span>
              </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#d7ccc8] text-sm">Total</p>
                <p className="text-2xl font-bold text-[#ffccbc]">{stats.total}</p>
              </div>
              <div className="w-10 h-10 bg-[#795548]/20 rounded-xl flex items-center justify-center">
                <CheckCircle className="w-5 h-5 text-[#ffccbc]" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#d7ccc8] text-sm">Completed</p>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#d7ccc8] text-sm">Pending</p>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#d7ccc8] text-sm">Overdue</p>
            ...
            {['all', 'pending', 'completed'].map((filterType) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl font-medium transition-all capitalize ${
                  filter === filterType
                    ? 'bg-[#795548] text-[#ffccbc] shadow-lg'
                    : 'bg-white/10 text-[#d7ccc8] hover:bg-white/20'
                }`}
              >
                {filterType} {filterType === 'all' ? `(${stats.total})` : filterType === 'pending' ? `(${stats.pending})` : `(${stats.completed})`}
              </button>
        ...
        {filteredTasks.length === 0 ? (
          <div className="text-center py-16">
            <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle className="w-8 h-8 text-[#d7ccc8]" />
            </div>
            <h3 className="text-xl font-semibold text-[#ffccbc] mb-2">
              {filter === 'all' ? 'No tasks yet' : `No ${filter} tasks`}
            </h3>
